...
import com.jk1123.mock.util.Text2ListUtil;
 ...
 * @description
 */
public class MockChineseNameGenerator {
    private static final Random RANDOM = new Random(System.currentTimeMillis());
    private static Logger logger = LoggerFactory.getLogger(MockChineseNameGenerator.class);
    private static List<String> firstNames = null;
    private static List<String> secondBoyNames = null;
    private static List<String> secondGirlNames = null;
    private static String COMMON =
                    ...
                    "罐";
    static {
        logger.debug("开始加载中文人名-姓氏模拟数据");
        long start = System.currentTimeMillis();

        firstNames = Text2ListUtil.transferClassPath("mock/name/chinese/family.txt");
        secondBoyNames = Text2ListUtil.transferClassPath("mock/name/chinese/boy-names.txt");
        secondGirlNames = Text2ListUtil.transferClassPath("mock/name/chinese/girl-names.txt");
        long end = System.currentTimeMillis();
        logger.debug("加载中文人名-姓氏模拟数据完成,花费了{}毫秒", end - start);
    }

    public static String generate() {
        return RANDOM.nextInt(2) == 0 ? generateRandom() : generatePrepare();
    }

    public static String generateRandom() {
        StringBuffer stringBuffer = new StringBuffer();
        stringBuffer.append(firstNames.get(RANDOM.nextInt(firstNames.size())));
        int count = RANDOM.nextInt(2) + 1;
        for (int i = 0; i < count; i++) {
            //char c = (char) (0x4e00 + (int) (Math.random() * (0x9fa5 - 0x4e00 + 1)));
            char c = COMMON.charAt(RANDOM.nextInt(COMMON.length()));
            stringBuffer.append(c);
        }
        return stringBuffer.toString();
    }

    public static String generatePrepare() {
        return RANDOM.nextInt(2) == 0 ? generateBoyName() : generateGirlName();
    }

    public static String generateBoyName() {
        return firstNames.get(RANDOM.nextInt(firstNames.size())) + secondBoyNames.get(RANDOM.nextInt(secondBoyNames.size()));
    }

    public static String generateGirlName() {
        return firstNames.get(RANDOM.nextInt(firstNames.size())) + secondGirlNames.get(RANDOM.nextInt(secondGirlNames.size()));
    }
